...
class RouteTest extends PHPUnit_Framework_TestCase {
    ...
    public function testStatic() {
        $route = new PatternRoute('action', null, '');
        $this->assertTrue($route->match(''));
        $this->assertFalse($route->match('a'));

        $route = new PatternRoute('action', null, 'contact');
        $this->assertTrue($route->match('contact'));
    }

    public function testRequiredOnly() {
        $route = new PatternRoute('action', null, 'news/{id:int}');
        $this->assertTrue($route->match('news/18'));

        $route = new PatternRoute('action', null, 'news/{id:int}/{txt}');
    ...
    public function testOptionals() {
        ...
        $route = new PatternRoute('action', null, 'news/{id:int}/{txt:any:default}');
        $this->assertTrue($route->match('news/18/asd_asd-asdsa.asd'));
        $this->assertFalse($route->match('news/18a/asd_asd-asdsa.asd'));
        $this->assertFalse($route->match('news//asd_asd-asdsa.asd'));
        ...
        $route = new PatternRoute('action', null, 'news/{category:int:0}/{page:int:1}/{order:int:}');
        $this->assertTrue($route->match('news/18/12/0'));
        $this->assertTrue($route->match('news///'));
        $this->assertTrue($route->match('news'));
        $this->assertFalse($route->match('newssss'));

        $route = new PatternRoute('action', null, 'news/{page:int:1}/{order:any:}');
        $this->assertTrue($route->match('news'));
        $this->assertFalse($route->match('newsssss'));
    }

    public function testAdvanced() {
        $route = new PatternRoute('action', null, 'page/{path:.+}');
        $this->assertTrue($route->match('page/page1/page2'));
        $this->assertFalse($route->match('page'));
        $this->assertFalse($route->match('page/'));

        $route = new PatternRoute(
                'action', null, 'news/{year}-{month}/{page:int:1}', array(
            'year' => array('reqs' => '\d{4}'),
        ...
        $route = new PatternRoute('action', null, 'sitemap/{test}.xml');
        ...
        $route = new PatternRoute('action', null, 'sitemap', array('ext' => array('default' => 'html')));
        $this->assertTrue($route->match('sitemap', 'GET', $m));
        $this->assertEquals($m['ext'], 'html');

        // other method to change requirement
        $route = new PatternRoute('action', null, 'news/{id:int}', array('id' => array('required' => false)));
        $this->assertTrue($route->match('news/18'));
        $this->assertTrue($route->match('news/'));
        $this->assertTrue($route->match('news'));
        $this->assertFalse($route->match('news/123a'));
    ...
    public function testUrls() {
        $route = new PatternRoute('action', null, 'user/{login}');
        $this->assertEquals($route->url(array('login' => 'admin')), 'user/admin');

        $route = new PatternRoute('action', null, 'news/{year:int}-{month:int}/{page:int:1}');
